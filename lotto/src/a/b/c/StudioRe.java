...
public class StudioRe {
	public static void main(String[] args) {
		//  실행하기 위해서는 studio 자체 생성을 먼저 한후 메소드를 사용 
		StudioRe sbs= new StudioRe();
		sbs.onAir(); //객체 생성을 했으니 객체 안에서 함수를 써야지
		
	}
	
	public LottoMachineRe ready() {
		LottoMachineRe machine = new LottoMachineRe(); // 메인함수 클래스있는 곳에는 property를 직접 생성을 피하기 위해 사용하는 클래스의 객체를 클래스 안에 생성해서 사용
	...
	public void onAir() {
		...
		//ready 함수를 쓰기 위해서 선언
		LottoMachineRe machine = this.ready(); // new 로 생성하지 않아도 되는건지?-> this로 이미 sbs 객체를 통하여 machine이 생성 되었기 때문에 그래서 아래의 macine.startMachine 쓸 수 있는것임
